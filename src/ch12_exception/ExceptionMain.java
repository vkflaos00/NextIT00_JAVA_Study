...
public class ExceptionMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 클래스. 해당 에러가 발생하면 catch 블록 내부 실행
			System.out.println("예외 발생!!!");
			System.out.println("에러메시지: " + except.getMessage());
			
			// 에러 로그 출력 (항상 보던 거)
			// System.err.println() 으로 출력됨.
			// syso랑 출력되는 타이밍이 지 맘대로임
...
//		}
		...
		try {
			ExMethod.printName("");
		} catch (BizException e) { // throw new BizException 들어온다
			e.printStackTrace();
		...
		try {
			System.out.println(ExMethod.dateMillSec("2023.04.21"));
