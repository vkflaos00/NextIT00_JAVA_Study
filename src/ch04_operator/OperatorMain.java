...
public class OperatorMain {
	...
	public static void main(String[] args) {
		...
		int showCountOnePage = 10; // 한 페이지에서 보여줄 글의 수가 10개
		...
		int checkInt2 = (inputId.length()) > 0 ? 1 : 0 ; // 괄호가 없어도 동작함 
		System.out.println(checkInt);
		System.out.println(checkInt2);
		...
		// 조건 체크를 여러 번 하지 않고, 한 번에 끝낼 수 있도록 도와줌
		...
		// 10<= x <= 11
		//System.out.println(10 <= inputPhone.length() <= 11); // 이건 자바에서는 동작하지 않음. 하기 방식으로 해야함
		System.out.println(10 <= inputPhone.length() && inputPhone.length() <= 11);
		
		// || 
		// 좌우 조건 중 하나라도 true면 true 리턴
		// 좌우 조건 둘 다 false일때만 false 리턴
		...
		// 명령어 chmod 755 읽기 쓰기 실행 권한을 사용자 그룹 게스트로 111 101 101
		
		// 16진수 사용 예시
		// RGB컬러값 255.000.000 < R #FF0000 <- 둘 다 RED값
		
		System.out.println("\n=========================\n");
		
		// 면접에서 물어본 2진수, 16진수 문제
		// A. 16진수 12f 를 10진수로 바꾼 후 2진수로 변환 // 303 // 100101111
		// B. 10진수 2진수로 변환 
